package com.bbva.ccol.lib.r005.impl;
...
/**
 * The CCOLR005Impl class...
 */
public class CCOLR005Impl extends CCOLR005Abstract {

	private static final Logger LOGGER = LoggerFactory.getLogger(CCOLR005Impl.class);
	 ...
	 */
	@Override
	public List<EmployeeDTO> executeListCustomer(PaginationIn paginationIn) {
		List<Map<String, Object>> dato = null;
		Map<String, Object> manejoErrores = new HashMap<>();
		try {
			dato = this.jdbcUtils.pagingQueryForList("sql.find.listarempleados", Integer.parseInt( paginationIn.getPaginationKey()),  Integer.parseInt(paginationIn.getPageSize().toString()));
	...
	public Integer executeListEmployeePaginacion() {
		...
		try {
			dato = this.jdbcUtils.queryForString("sql.find.totalempleados", total);
		}catch (NoResultException e){
			dato = "-1";
			this.addAdviceWithDescription("CCOL00000001","error en conexión");
			LOGGER.info(e.getMessage());
